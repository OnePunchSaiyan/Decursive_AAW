...
do
    ...
    function D:GetClassColor(EnglishClass)
        if not EnglishClass then
            DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[Decursive] Warning: GetClassColor called with nil class")
        ...
        if not DC.ClassesColors[EnglishClass] then
            if RAID_CLASS_COLORS and RAID_CLASS_COLORS[EnglishClass] then
                DC.ClassesColors[EnglishClass] = {
                    RAID_CLASS_COLORS[EnglishClass].r,
                    RAID_CLASS_COLORS[EnglishClass].g,
                    RAID_CLASS_COLORS[EnglishClass].b
                }
            else
                DC.ClassesColors[EnglishClass] = { 0.63, 0.63, 0.63 }
            end
    
            if LC and LC[EnglishClass] then
